	...
	dir1, s1 := testServerWithConfig(t, func(c *Config) {
		...
		c.CAConfig = &structs.CAConfiguration{
			...
			Config: map[string]interface{}{
				...
				// triggers definitely.
				"IntermediateCertTTL": 2 * time.Second,
			},
		}
	})
	defer os.RemoveAll(dir1)
...
func TestLeader_moreThanHalfTimePassed(t *testing.T) {
	now := time.Now()
	require.True(t, moreThanHalfTimeHasPassed(now, now.Add(-10*time.Second), now.Add(-5*time.Second)))
	require.True(t, moreThanHalfTimeHasPassed(now, now.Add(-10*time.Second), now))
	require.True(t, moreThanHalfTimeHasPassed(now, now.Add(-10*time.Second), now.Add(5*time.Second)))

	require.False(t, moreThanHalfTimeHasPassed(now, now.Add(-10*time.Second), now.Add(10*time.Second)))
	require.False(t, moreThanHalfTimeHasPassed(now, now.Add(-10*time.Second), now.Add(20*time.Second)))
}
